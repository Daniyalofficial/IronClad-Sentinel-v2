...
ignore-rules/baseline filtering, and computes summary statistics. This
is the single entry point the CLI (and any future integration, e.g. a
pre-commit hook or IDE plugin) calls into.
"""
...
from ironclad.core.models import Engine, Finding, ScanResult, ScanStats, Severity
...
def run_scan(config: IronCladConfig, progress_callback=None) -> ScanResult:
    ...
    if "iac" in config.enabled_engines:
        ...
        all_findings.extend(scan_iac_files(fileset.iac_files()))
    ...
    # Filters: ignored rule IDs, ignored paths, minimum severity
    filtered = []
    ...
    # De-duplicate by fingerprint (same underlying issue caught by two engines)
    seen = set()
    ...
    stats = ScanStats(
        files_scanned=len(fileset.files),
        files_skipped=fileset.skipped,
